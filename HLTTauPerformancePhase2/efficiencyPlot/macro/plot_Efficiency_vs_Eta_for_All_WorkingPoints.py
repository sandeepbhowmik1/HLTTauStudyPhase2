...
for k in range (0, len(plotRanges)):
    ...
    for i in range (0, len(workingPoints)):
        ...
        if(count==5):
            count+=1
        hist_HLTTau.append(fileIn_HLTTau.Get("histo_Phase2_HLTTau_"+workingPoints[i]))
        hist_HLTTau[-1].__class__ = ROOT.RooHist
        fit_HLTTau.append(fileIn_HLTTau.Get("fit_Phase2_HLTTau_"+workingPoints[i]))
    ...
    plots[-1].extraText3 = "gen #tau_{h} p_{T} > 20 GeV"
